...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    def _load_image(self, directory, idx):
        files = [f for f in os.listdir(directory) if re.match(r'\d{6}.jpg', f)]
        return [Image.open(os.path.join(directory, files[idx])).convert('RGB')]
        #return [Image.open(os.path.join(directory, self.imagefile_template.format(idx))).convert('RGB')]

    def _parse_list(self):
        print([VideoRecord(x.strip().split(' '), self.root_path) for x in open(self.annotationfile_path)])
    ...
    def _get(self, record, indices):
        ...
        return images, torch.tensor(record.label)
